...
public class BotRuntime {
    ...
    public void start(String reason) {
        log.info("{}Starting bot runtime id={} name={} family={} strategy={} strategySet={} subStrategy={} reason={}{}",
                LogColors.MARKET, botId(), config.getName(), config.getMarketFamily(), config.getStrategyId(), config.getStrategySetId(), config.getSubStrategyId(), reason, LogColors.RESET);
        eventLogger.market(
                "BOT_RUNTIME_STARTED",
                botId(),
                null,
                reason,
                TelemetryData.data("name", config.getName(), "family", config.getMarketFamily(), "strategyId", config.getStrategyId(), "strategySetId", config.getStrategySetId(), "subStrategyId", config.getSubStrategyId()),
                true
        );
